...
public class SutPathsFiles {
    ...
    public List<String> getSuts() {
        final List<String> sutNames = new ArrayList<>();
    ...
    /**
     * Get the names of report files with/without path prefix depending on withPath
     * parameter
     *
     * @param sutId the ID of the desired SUT
    ...
    public List<String> getSutReportFileNames(final String sutId, final boolean withPath) {
        ...
        final String path = getReportPath(sutId);
    ...
    /**
     * Get report file names with/without path prefix depending on withPath
     * parameter
     *
    ...
    private List<String> listReportFilesForFolder(final File folderName, final String path, final boolean withPath) {
        final List<String> reportFileNames = new ArrayList<>();
        if (folderName == null) {
            return reportFileNames;
        }
    ...
    /**
     * Get the names of the TcParam files available without path prefix. Currently
     * only one, may change in the future.
     *
     * @param sutId the desired SUT ID
    ...
    /**
     * Get the names of the TcParam files available with/without path prefix
     * depending on withPath parameter. Currently only one, may change in the future
     *
     * @param sutId the desired SUT ID
     * @param testsuiteName the name of the testsuite under consideration
    ...
    public List<String> getTcParamFileNames(final String sutId, final String testsuiteName, final boolean withPath) {
        final List<String> tcParamFileNames = new ArrayList<>();

        final String folderName = getTcParamPath(sutId, testsuiteName);
        if (folderName == null) {
    ...
    /**
     * Get the names of log files with/without path prefix depending on withPath
     * parameter
     *
     * @param sutId the ID of the SUT
     * @param testsuiteName the name of the testsuite under consideration
    ...
    public List<String> getSutLogFileNames(final String sutId, final String testsuiteName, final boolean withPath) {
        ...
        final String path = getSutLogPathName(sutId, testsuiteName);
        final File folder = new File(path);
        if (folder.exists() == false || folder.isDirectory() == false) {
            return logFileNames;
        }
    ...
    public String getSutLogPathName(final String sutId, final String testsuiteName) {
        final String sutsHome = getSutsHomePath();
        if (sutsHome == null) {
            return null;
        }
